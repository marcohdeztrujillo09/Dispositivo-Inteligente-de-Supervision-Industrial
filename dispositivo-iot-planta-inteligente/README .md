...
Este proyecto está estructurado de forma sencilla y modular para facilitar su replicación y mantenimiento, adaptado completamente para trabajar con **Arduino IDE** y Docker:

```text
├── 📁 capturas/                # Material visual (Fotos, capturas de pantalla y vídeos de demostración)
├── 📁 hardware/                # Planos físicos y esquemas del circuito
│   └── 📄 esquema-electrico.pdf # Esquema de conexiones en alta resolución
├── 📁 esp32/                   # Firmware del microcontrolador (Arduino IDE)
│   ├── 📄 esp32.ino            # Código fuente principal del proyecto
│   └── 📄 config.h             # Credenciales privadas (WiFi/MQTT) *Ignorado en Git por seguridad*
...
│   ├── 📁 data/                # Datos persistentes de los mensajes *Ignorado en Git*
│   └── 📁 log/                 # Registros de actividad del bróker *Ignorado en Git*
├── 📄 docker-compose.yml       # Orquestación de infraestructura local (Docker)
└── 📄 .gitignore               # Filtro para evitar subir datos locales de Docker o contraseñas
...
| Capa | Tecnología |
|---|---|
| **Hardware** | ESP32 + sensores (BME280, MQ, KY-038, LDR) |
| **Protocolo** | MQTT |
| **Automatización** | n8n |
...
- El mensaje en Telegram incluye todos los valores del sensor, la anomalía detectada y el diagnóstico IA.
